...
def evaluar_funcion(x):
    ...
    funciones = {
        ...
        # --- NUEVO: Si elige seno, el programa usa la función seno ya definida en la sección 2.
        "seno": lambda x: seno(x),
        # --- NUEVO: Si elige coseno, el programa usa la función coseno ya definida en la sección 2.
        "coseno": lambda x: coseno(x),
    }
...
# --- MODIFICADO: se agregó nombre_funcion2=None para poder recibir una segunda función opcional.
def graficar_funcion(nombre_funcion, rango_inicio, rango_fin, altura, nombre_funcion2=None):
    ...
    # --- NUEVO: Si el usuario pidió una segunda función, la buscamos y verificamos que exista.
    funcion2 = None
    if nombre_funcion2 is not None:
        if nombre_funcion2 not in funciones:
            print("Error: Segunda función no encontrada")
            return
        # Tomamos la segunda fórmula del diccionario para poder calcular sus puntos.
        funcion2 = funciones[nombre_funcion2]
    ...
    valores = []
    # --- NUEVO: Preparamos un segundo espacio para los puntos de la segunda función.
    valores2 = []
    ...
    while x <= rango_fin:
        ...
        valores.append(y)
        # --- NUEVO: Si hay segunda función, calculamos y guardamos su punto para el mismo x.
        if funcion2 is not None:
            y2 = funcion2(x)
            valores2.append(y2)
    ...
    # --- NUEVO: Si hay segunda función, ajustamos el máximo y mínimo para que ambas curvas
    # --- NUEVO: queden en la misma escala vertical y se puedan comparar correctamente.
    if valores2:
        if max(valores2) > valor_maximo:
            valor_maximo = max(valores2)
        if min(valores2) < valor_minimo:
            valor_minimo = min(valores2)
    ...
    for i in range(altura):
        ...
        for j in range(len(valores)):
            ...
            # Si no coincide, dejamos el espacio en blanco para no manchar el dibujo.
                ...
                fila.append(" ")
            # --- NUEVO: Revisamos si la segunda función también tiene un punto en esta misma celda.
            if funcion2 is not None and j < len(valores2):
                valor_normalizado2 = (valores2[j] - valor_minimo) / (valor_maximo - valor_minimo) * (altura - 1)
                if int(valor_normalizado2) == altura - 1 - i:
                    # Si la primera función también estaba aquí, marcamos el cruce con @.
                    if fila[-1] == "*":
                        fila[-1] = "@"
                    # Si solo está la segunda función en esta celda, usamos el símbolo +.
                    else:
                        fila[-1] = "+"
        # Cuando terminamos de revisar el renglón entero, lo guardamos en la cuadrícula.
        matriz.append(fila)
    
    # --- NUEVO: Si hay dos funciones, mostramos la leyenda antes del dibujo.
    # --- NUEVO: Esto le dice al usuario qué símbolo representa cada curva.
    if funcion2 is not None:
        print(f"Leyenda:  * = {nombre_funcion}   + = {nombre_funcion2}   @ = se cruzan")
        print()
...
def menu_graficar_funcion():
    ...
    while True:
        ...
        # --- NUEVO: Preguntamos si el usuario quiere agregar una segunda función a la gráfica.
        print("Desea graficar una segunda funcion al mismo tiempo?")
        print("0. No, solo una funcion")
        for i, nf in enumerate(lista_funciones):
            print(f"{i + 1}. {nf}")
        seleccion2_str = input("Seleccione la segunda funcion o 0 para omitir: ")
        # --- NUEVO: Aplicamos el mismo filtro de seguridad a esta nueva entrada.
        if not es_numero_valido(seleccion2_str, "int"):
            print("Entrada invalida.")
            continue
        seleccion2 = int(seleccion2_str)
        # --- NUEVO: Inicializamos la segunda función en None (vacío) por defecto.
        nombre_funcion2 = None
        # --- NUEVO: Solo asignamos la segunda función si el usuario eligió una opción válida distinta de 0.
        if seleccion2 != 0:
            if seleccion2 < 1 or seleccion2 > len(lista_funciones):
                print("Opcion no valida.")
                continue
            nombre_funcion2 = lista_funciones[seleccion2 - 1]
        ...
        # --- MODIFICADO: ahora también pasamos nombre_funcion2 (puede ser None si solo es una función).
        graficar_funcion(nombre_funcion, rango_inicio, rango_fin, altura, nombre_funcion2)
        # Aseguramos el comprobante del gráfico en el arreglo principal.
        # --- NUEVO: Si hubo segunda función, el historial lo registra también.
        if nombre_funcion2 is not None:
            historial.append(f"Grafica doble: {nombre_funcion} y {nombre_funcion2} de {rango_inicio} a {rango_fin}")
        else:
